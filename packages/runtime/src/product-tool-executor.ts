...
function assertAllowedTool(toolName: string, allowedTools: readonly string[]): void {
  if (!allowedTools.includes(toolName)) throw new Error(`Unauthorized agent tool: ${toolName}`);
}
...
export class AgentToolExecutor {
  ...
  async executeTool<I = unknown, O = unknown>(input: AgentToolExecutionInput<I>): Promise<O> {
    ...
    await this.deps.stores.eventTraceStore.append({ id: newId('evt'), runId: input.runId, type: 'tool.started', payload: { toolName: input.toolName, skillId: tool.skill.id, skillVersion: tool.skill.version, operationId: input.operationId, agentSessionId: input.agentSession.id }, createdAt: nowIso() });
    try {
      ...
      await this.deps.stores.eventTraceStore.append({ id: newId('evt'), runId: input.runId, type: 'tool.completed', payload: { toolName: input.toolName, skillId: tool.skill.id, skillVersion: tool.skill.version, operationId: input.operationId, agentSessionId: input.agentSession.id }, createdAt: nowIso() });
      return parsedOutput;
    } catch (error) {
      ...
      await this.deps.stores.eventTraceStore.append({ id: newId('evt'), runId: input.runId, type: 'tool.failed', payload: { toolName: input.toolName, skillId: tool.skill.id, skillVersion: tool.skill.version, operationId: input.operationId, agentSessionId: input.agentSession.id, error: message }, createdAt: nowIso() });
      throw error;
    }
  }
}
