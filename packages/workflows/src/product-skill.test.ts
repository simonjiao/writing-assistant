...
import { z } from 'zod';
import { loadDefaultProductSkills, loadProductSkillsFromDirectory, parseProductSkillMarkdown } from './product-skill';
import { ToolRegistry } from './product-tool';
...
describe('product skills', () => {
  it('loads Markdown product skills as strict runtime specs', () => {
    const skills = loadDefaultProductSkills();
    expect(skills.length).toBeGreaterThanOrEqual(10);
    const createTaskCard = skills.find((skill) => skill.id === 'create-task-card');
    expect(createTaskCard).toMatchObject({
      title: '创建任务卡',
      version: 1,
      toolBindings: ['create_task_intake', 'refine_task_card'],
    });
    expect(createTaskCard?.actionHints.create_task_intake).toContain('保存任务容器');
  });
