...
const actionSkillBindings: Record<AllowedActionType, { skillId: string; toolName?: string }> = {
...
const productSkillSchema = z.object({
  ...
  actionHints: z.record(z.string().min(1)),
}).strict();

let defaultSkillCache: ProductSkillSpec[] | undefined;

export function registerDefaultProductSkills(registry = new ProductSkillRegistry()): ProductSkillRegistry {
  for (const skill of loadDefaultProductSkills()) registry.register(skill);
  return registry;
}

export function loadDefaultProductSkills(): ProductSkillSpec[] {
  defaultSkillCache ??= loadProductSkillsFromDirectory(resolveDefaultProductSkillDirectory());
...
export class ProductSkillRegistry {
  ...
  register(skill: ProductSkillSpec): void {
    ...
    for (const actionType of Object.keys(skill.actionHints)) {
      const binding = actionSkillBindings[actionType as AllowedActionType];
      if (!binding) throw new Error(`Product skill ${skill.id} has unknown action hint: ${actionType}`);
      if (binding.skillId !== skill.id) throw new Error(`Product skill ${skill.id} cannot define action hint for ${actionType}; owner is ${binding.skillId}.`);
    }
...
export function actionSkillBinding(type: AllowedActionType): { skillId: string; skillVersion: number; toolName?: string; hint: string } {
  const binding = actionSkillBindings[type];
  const skill = defaultProductSkillRegistry().get(binding.skillId);
  return { ...binding, skillVersion: skill.version, hint: skill.actionHints[type] ?? skill.goal };
}

function defaultProductSkillRegistry(): ProductSkillRegistry {
  return registerDefaultProductSkills();
}

function resolveDefaultProductSkillDirectory(): string {
  const candidates = [
    resolve(__dirname, 'product-skills'),
    resolve(dirname(__dirname), 'product-skills'),
  ];
  const directory = candidates.find((candidate) => existsSync(candidate));
  if (!directory) throw new Error(`Product skill directory not found. Tried: ${candidates.join(', ')}`);
  return directory;
}
