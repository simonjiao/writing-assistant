...
## 输出契约
只返回一个 JSON object，字段必须为 taskCard、summary、missingQuestions、followUpPrompts、changedFields。
taskCard 必须是完整 WritingTaskCard；不要只返回局部 patch。
taskCard.structure.articleType 只能是 essay、analysis、commentary、speech、longform。
summary 是本次改动摘要，必须非空。
missingQuestions 是仍需用户补充的问题，没有则输出 []。
followUpPrompts 是 Array<{ question: string; options: string[]; allowCustom: boolean; selectionMode?: "single" | "multi" }>，需要和 missingQuestions 对应，没有则输出 []。
