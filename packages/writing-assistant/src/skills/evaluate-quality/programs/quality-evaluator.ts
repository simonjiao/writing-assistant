...
export class QualityEvaluatorProgram implements PromptProgram<QualityEvaluatorInput, QualityEvaluatorOutput> {
  manifest = {
  ...
  async invoke({ input, context }: Parameters<PromptProgram<QualityEvaluatorInput, QualityEvaluatorOutput>['invoke']>[0]): Promise<QualityEvaluatorOutput> {
