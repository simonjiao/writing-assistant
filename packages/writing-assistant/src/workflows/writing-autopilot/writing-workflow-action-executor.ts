...
import { consistencyReviewSignature } from './allowed-actions';
import { AgentSessionTarget, AgentToolExecutor, getOrCreateAgentSession } from '@wa/runtime';
import { processArticleComments } from '../../skills/resolve-article-comment/tools/article-comments';
import { OutlinePlannerInput, OutlinePlannerOutput } from '../../skills/plan-outline/programs/outline-planner';
import { SectionWriterInput, SectionWriterOutput } from '../../skills/write-section/programs/section-writer';
import { TaskCardBuilderInput, TaskCardBuilderOutput } from '../../skills/create-task-card/programs/task-card-builder';
...
type WorkflowToolRegistry = Readonly<Record<string, WorkflowActionHandler>>;

export class WritingAutopilotActionExecutor implements WorkflowActionExecutor {
  ...
  private async executeAction(run: WorkflowRun, action: AllowedAction): Promise<WorkflowActionExecutionResult> {
    const handler = this.tools[action.type];
    if (!handler) throw new Error(`WritingAutopilotActionExecutor has no handler for action: ${action.type}`);
  ...
  private async executeWorkflowTool<I = unknown, O = unknown>(run: WorkflowRun, action: AllowedAction, input: {
    ...
    return this.deps.agentToolExecutor.executeTool<I, O>({
