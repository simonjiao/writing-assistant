...
import { ActionCatalog, WorkflowPlannerWaitState } from '@wa/runtime';
import { createWritingAssistantActionCatalog } from '../../product';
...
export class AllowedActionPlanner {
  constructor(private readonly actionCatalog: ActionCatalog = createWritingAssistantActionCatalog()) {}

  shouldWait(input: AllowedActionPlannerInput): WorkflowPlannerWaitState | undefined {
    const { run, article } = input;
    if (isWaitingForCurrentRevisionProposal(run, article)) {
      return { reason: '已生成待确认修改方案，请先应用或取消后再继续写作。', nodeId: 'revision-proposal' };
    }
    if (isBlockedByCurrentConsistencyReview(run, article) && !hasPendingReviewProposal(run, article)) {
      return { reason: '一致性检查发现阻断问题，请先处理右侧建议后再继续写作。', nodeId: 'consistency-review' };
    }
    return undefined;
  }
  ...
  private action(run: WorkflowRun, type: AllowedAction['type'], input: { article?: ArticleArtifact; section?: OutlineItem; reason: string; requiresHumanGate?: boolean; reviewArtifactId?: string; suggestionId?: string; targetKind?: string; targetId?: string }): AllowedAction {
    ...
    const skillBinding = this.actionCatalog.require(type);
