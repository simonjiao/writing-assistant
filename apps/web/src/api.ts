import { AgentEvent, ArticleArtifact, ArticleSummary, DomainProfileRecommendation, DomainProfileSelection, DomainProfileSummary, RunResponse, WritingWorkspace } from './types';
...
export const api = {
  ...
  startTaskCard(rawRequirement: string, userId: string, sessionId?: string, workspaceId?: string, domainProfile?: DomainProfileSelection) { return request<RunResponse>('/api/workflows/task-card/start', { method: 'POST', body: JSON.stringify({ rawRequirement, userId, sessionId, workspaceId, domainProfile }) }); },
