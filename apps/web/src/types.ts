export interface TaskCardFollowUpPrompt { id: string; question: string; options: string[]; allowCustom: boolean; selectionMode?: 'single' | 'multi' }
...
export interface RevisionProposal { id: string; articleId: string; userId: string; runId?: string; authorUserId?: string; appliedByUserId?: string; contextKind: DialogueContextKind; summary: string; message: string; operations: RevisionOperation[]; warnings: string[]; status: 'pending' | 'applied' | 'dismissed'; createdAt: string; updatedAt: string }
...
export interface DialogueResponse { mode: 'answer' | 'clarify' | 'discuss' | 'proposal' | 'applied'; message: string; proposal?: RevisionProposal; article?: ArticleArtifact; run?: WorkflowRun; events?: AgentEvent[]; humanGates?: HumanGate[]; operations?: AgentOperation[]; reviewArtifacts?: ReviewArtifact[]; revisionProposals?: RevisionProposal[]; messages?: DialogueMessage[] }
...
export interface WorkflowRun { id: string; workflowId: string; status: 'idle' | 'running' | 'waiting' | 'completed' | 'failed' | 'cancelled'; waitingFor?: { nodeId: string; reason: string }; state: Record<string, unknown>; error?: string }
...
export interface AgentOperation { operationId: string; runId?: string; userId: string; articleId?: string; toolName: string; allowedActionId: string; argsHash: string; status: 'running' | 'completed' | 'failed'; resultRef?: string; error?: string; articleRevisionBefore?: number; articleRevisionAfter?: number; createdAt: string; updatedAt: string }
...
export interface RunResponse { run: WorkflowRun; article?: ArticleArtifact; events: AgentEvent[]; humanGates?: HumanGate[]; operations?: AgentOperation[]; reviewArtifacts?: ReviewArtifact[]; revisionProposals?: RevisionProposal[]; messages?: DialogueMessage[] }
