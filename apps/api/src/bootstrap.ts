...
function registerWorkflows(engine: WorkflowEngine, deps: { artifactStore: ArtifactStore; sessionStore: SessionStore; eventTraceStore: EventTraceStore }): void {
  ...
  const outlineWorkflow: WorkflowDefinition = {
    ...
    nodes: [
      { id: 'plan-outline', label: '生成大纲', kind: 'skill', skillId: 'outline-planner', input: async ({ run }) => { const input = run.input as { articleId: string }; const article = await deps.artifactStore.getArticle(input.articleId); if (!article?.taskCard) throw new Error('Task card is required before outlining.'); if (article.taskCard.status !== 'confirmed') throw new Error('Task card must be confirmed before outlining.'); const normalized = normalizeTaskCardPolicies(article.taskCard); if (normalized.changed) { article.taskCard = normalized.taskCard; await deps.artifactStore.updateArticle(article); } return { articleId: article.id, taskCard: article.taskCard }; }, outputKey: 'outlineResult', next: 'commit-outline-draft' },
  ...
  const sectionWorkflow: WorkflowDefinition = {
    ...
    nodes: [
      { id: 'write-section', label: '生成章节正文', kind: 'skill', skillId: 'section-writer', input: async ({ run }) => { const input = run.input as { articleId: string; sectionId: string }; const article = await deps.artifactStore.getArticle(input.articleId); if (!article?.taskCard) throw new Error('Task card is required.'); if (article.taskCard.status !== 'confirmed') throw new Error('Task card must be confirmed before writing.'); const normalized = normalizeTaskCardPolicies(article.taskCard); if (normalized.changed) { article.taskCard = normalized.taskCard; await deps.artifactStore.updateArticle(article); } const section = article.outline.find((item) => item.id === input.sectionId); if (!section) throw new Error('Section not found.'); if (section.status === 'draft') throw new Error('Outline must be ready before writing.'); return { articleId: article.id, section, taskCard: article.taskCard }; }, outputKey: 'sectionResult', next: 'commit-section' },
