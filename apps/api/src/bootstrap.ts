...
function registerWorkflows(engine: WorkflowEngine, deps: { artifactStore: ArtifactStore; sessionStore: SessionStore; eventTraceStore: EventTraceStore }): void {
  ...
  const outlineWorkflow: WorkflowDefinition = {
    id: 'outline-workflow', name: '大纲生成流程', description: '基于任务卡生成大纲，并等待用户确认。', startNodeId: 'plan-outline',
    nodes: [
      { id: 'plan-outline', label: '生成大纲', kind: 'skill', skillId: 'outline-planner', input: async ({ run }) => { const input = run.input as { articleId: string }; const article = await deps.artifactStore.getArticle(input.articleId); if (!article?.taskCard) throw new Error('Task card is required before outlining.'); return { articleId: article.id, taskCard: article.taskCard }; }, outputKey: 'outlineResult', next: 'commit-outline-draft' },
      { id: 'commit-outline-draft', label: '保存大纲草稿', kind: 'function', outputKey: 'outlineDraft', handler: async ({ run }) => { const input = run.input as { articleId: string }; const result = run.state.outlineResult as { outline: [] }; const article = await deps.artifactStore.getArticle(input.articleId); if (!article) throw new Error('Article not found.'); article.outline = result.outline; await deps.artifactStore.updateArticle(article); await deps.eventTraceStore.append({ id: newId('evt'), runId: run.id, type: 'artifact.updated', payload: { articleId: article.id, reason: 'outline-draft-created', userId: run.metadata.userId }, createdAt: nowIso() }); return { articleId: article.id, outline: article.outline }; }, next: 'wait-outline-confirm' },
      { id: 'wait-outline-confirm', label: '等待用户确认大纲', kind: 'wait', reason: '请确认或调整大纲。', next: 'finalize-outline' },
      { id: 'finalize-outline', label: '确认大纲', kind: 'function', outputKey: 'finalizedOutline', handler: async ({ run }) => { const draft = run.state.outlineDraft as { articleId: string }; const response = (run.state['wait-outline-confirmResponse'] ?? {}) as { outline?: [] }; const article = await deps.artifactStore.getArticle(draft.articleId); if (!article) throw new Error('Article not found.'); if (response.outline) article.outline = response.outline; article.outline = article.outline.map((item) => ({ ...item, status: 'confirmed' as const })); await deps.artifactStore.updateArticle(article); await deps.artifactStore.commitVersion(article.id, '确认大纲', 'user'); return { articleId: article.id, outline: article.outline }; }, next: END },
    ],
  };
  ...
  const sectionWorkflow: WorkflowDefinition = {
    ...
    nodes: [
      { id: 'write-section', label: '生成章节正文', kind: 'skill', skillId: 'section-writer', input: async ({ run }) => { const input = run.input as { articleId: string; sectionId: string }; const article = await deps.artifactStore.getArticle(input.articleId); if (!article?.taskCard) throw new Error('Task card is required.'); const section = article.outline.find((item) => item.id === input.sectionId); if (!section) throw new Error('Section not found.'); return { articleId: article.id, section, taskCard: article.taskCard }; }, outputKey: 'sectionResult', next: 'commit-section' },
      { id: 'commit-section', label: '保存章节正文', kind: 'function', outputKey: 'committedSection', handler: async ({ run }) => { const input = run.input as { articleId: string; sectionId: string }; const result = run.state.sectionResult as { block: { id: string; sectionId?: string } }; const article = await deps.artifactStore.getArticle(input.articleId); if (!article) throw new Error('Article not found.'); article.blocks = article.blocks.filter((block) => block.sectionId !== input.sectionId).concat(result.block as never); article.outline = article.outline.map((item) => item.id === input.sectionId ? { ...item, status: 'written' as const } : item); await deps.artifactStore.updateArticle(article); await deps.artifactStore.commitVersion(article.id, `生成章节：${input.sectionId}`, 'agent'); await deps.eventTraceStore.append({ id: newId('evt'), runId: run.id, type: 'artifact.updated', payload: { articleId: article.id, reason: 'section-written', sectionId: input.sectionId, userId: run.metadata.userId }, createdAt: nowIso() }); return { articleId: article.id, blockId: result.block.id }; }, next: END },
    ],
  };
