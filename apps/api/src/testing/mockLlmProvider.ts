import { ChatRequest, ChatResponse, LLMProvider, safeJsonParse } from '@wa/core';
...
function mockDialogueRoute(payload: Record<string, any>) {
  ...
  if (mockNeedsKnowledgeSearch(message)) return { route: 'needs-rag' };
  if (/[?？]|为什么|为何|解释|说明|怎么|是否|吗|是什么|什么意思/.test(message)) return { route: 'answer' };
  if (mockIsModificationIntent(message)) return { route: 'propose' };
...
function mockDialogueCoordination(payload: Record<string, any>) {
  ...
  if (!mockIsModificationIntent(message)) {
