...
export class SqliteArtifactStore implements ArtifactStore {
  ...
  async createArticle(input: { id?: string; userId: string; workspaceId: string; title: string; taskCard?: ArticleArtifact['taskCard'] }) { if (input.id) { const existing = await this.getArticle(input.id); if (existing) return existing; } const now = nowIso(); const article: ArticleArtifact = { id: input.id ?? newId('art'), userId: input.userId, workspaceId: input.workspaceId, revision: 1, title: input.title, taskCard: input.taskCard, outline: [], blocks: [], citations: [], themeTags: [], comments: [], versions: [], createdAt: now, updatedAt: now }; await this.db.upsert(article); await this.commitVersion(article.id, '创建文章草稿', 'agent'); return (await this.getArticle(article.id)) as ArticleArtifact; }
